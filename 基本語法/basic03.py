"""
股票交易成本計算
小明在股價 10元時買了 Hero 公司股票 10 張, 於 11 元全數賣出,
請問小明賺了多少?
提示:
1張 = 1000股
交易成本(台股)
手續費:股票價值* 1.425(%) 買賣股票時都要扣除(不足 20 元者以 20元計算。)
證交稅:股票價值* 3(%) 賣出股票時才扣除
"""
import math
amount = 1 # 張數
cost = 10 * 10 * 1000 # 買進成本
...
fee_sell = math.floor (price * 0.001425)  # 賣出手續費
fee_sell = 20 if fee_sell < 20 else fee_sell #判斷手續費是否小於20元
tax_sell = math.ceil(price * 0.003) # 賣出交易稅
price_sub_fee_sell_tax_sell = price - \
                              fee_sell - tax_sell # 賣出價值 - 賣出價值 - 賣出手續交易稅
